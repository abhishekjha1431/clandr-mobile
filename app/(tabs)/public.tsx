...
import { Copy, ExternalLink, User, Clock } from 'lucide-react-native';
...
export default function PublicProfileScreen() {
  ...
  const userBio = 'Software Developer & Football Enthusiast. Book a session with me to discuss projects, grab coffee, or play some football!';
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        ...
        <View className="bg-white rounded-3xl shadow-lg p-6 mx-4 mt-6 mb-6">
          ...
          <View className="items-center mb-6">
            ...
            <Text className="text-2xl font-bold text-gray-800 mb-2">{userName}</Text>
            <Text className="text-gray-600 text-center leading-6 mb-4">{userBio}</Text>
          ...
          <View className="flex-row justify-around pt-4 border-t border-gray-100">
            <View className="items-center">
              <Text className="text-xl font-bold text-blue-600">{activeEvents.length}</Text>
        ...
        <View className="px-4 mb-6">
          <Text className="text-xl font-bold text-gray-800 mb-4">Available Events</Text>
          
          {activeEvents.length > 0 ? (
            activeEvents.map((event) => (
              <View key={event.id} className="bg-white rounded-3xl shadow-lg p-6 mb-4">
                {/* Event Header */}
                <View className="flex-row items-start justify-between mb-4">
                  <View className="flex-1">
                    ...
                    <View className="flex-row items-center mb-3">
                      ...
                      <Text className="text-gray-600 ml-2">{event.duration} minutes</Text>
                    </View>
                  </View>
                  <View className="bg-green-100 px-3 py-1 rounded-full">
                    <Text className="text-green-600 text-sm font-medium">Available</Text>
                  </View>
                </View>
                ...
                <View className="flex-row items-center justify-between mb-4 pt-4 border-t border-gray-100">
                  <Text className="text-gray-500 text-sm">{event.bookings} people booked this</Text>
            ...
            <View className="bg-white rounded-3xl shadow-lg p-8 items-center">
              <Text className="text-gray-500 text-lg mb-2">No events available</Text>
              <Text className="text-gray-400 text-center">This user hasn't created any bookable events yet</Text>
            </View>
          )}
        </View>
        ...
        <View className="px-4 mb-6">
          <View className="bg-white rounded-3xl shadow-lg p-6">
            <Text className="text-xl font-bold text-gray-800 mb-4">What people say</Text>
            
            {[
              { name: 'Sarah M.', text: 'Great football session! John is very friendly and skilled.', rating: 5 },
              { name: 'Mike R.', text: 'Excellent coding mentorship. Learned a lot in our session.', rating: 5 },
              { name: 'Emma K.', text: 'Perfect coffee chat for networking. Highly recommend!', rating: 4 }
            ].map((review, index) => (
              <View key={index} className="py-4 border-b border-gray-100 last:border-b-0">
                <View className="flex-row items-center justify-between mb-2">
                  <Text className="text-gray-800 font-semibold">{review.name}</Text>
                  <View className="flex-row">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Text key={star} className={`text-lg ${star <= review.rating ? 'text-yellow-400' : 'text-gray-300'}`}>
                        ★
                      </Text>
                    ))}
                  </View>
