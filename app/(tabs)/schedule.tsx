  ...
  const handleSlotPress = (day: string, time: string, status: string) => {
    ...
    Alert.alert(
      ...
      [
        ...
        { text: 'Edit', style: 'default', onPress: () => console.log('Edit slot') }
      ]
    );
  };
    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        ...
        <View className="pt-4 pb-6 px-4">
          ...
          <Text className="text-gray-600">Manage your availability and bookings</Text>
        </View>
          ...
          <View className="flex-row space-x-3 mb-3">
            <View className="bg-white rounded-3xl shadow-lg p-4 flex-1">
              <View className="flex-row items-center mb-2">
                ...
                <Text className="text-2xl font-bold text-green-600">{stats.availableSlots}</Text>
              </View>
            ...
            <View className="bg-white rounded-3xl shadow-lg p-4 flex-1">
              <View className="flex-row items-center mb-2">
                ...
                <Text className="text-2xl font-bold text-blue-600">{stats.bookedSlots}</Text>
              </View>
          ...
          <View className="bg-white rounded-3xl shadow-lg p-4">
            <View className="flex-row items-center justify-between">
              <View className="flex-row items-center">
                ...
                <Text className="text-gray-800 font-semibold">Total Weekly Slots</Text>
              </View>
              <Text className="text-2xl font-bold text-gray-800">{stats.totalSlots}</Text>
            </View>
          </View>
        </View>
        ...
        <View className="px-4 mb-6">
          <View className="bg-white rounded-3xl shadow-lg p-6">
            <Text className="text-xl font-bold text-gray-800 mb-4">Recent Bookings</Text>
            
            {[
              { name: 'Sarah Johnson', event: 'Coffee Chat ☕', time: 'Today, 2:00 PM', status: 'confirmed' },
              { name: 'Mike Chen', event: 'Play football⚽', time: 'Tomorrow, 10:00 AM', status: 'pending' },
              { name: 'Emma Wilson', event: 'Coding Session 💻', time: 'Friday, 3:00 PM', status: 'confirmed' }
            ].map((booking, index) => (
              <View key={index} className="py-4 border-b border-gray-100 last:border-b-0">
                <View className="flex-row items-center justify-between">
                  <View className="flex-1">
                    <Text className="text-gray-800 font-semibold mb-1">{booking.name}</Text>
                    <Text className="text-gray-600 text-sm">{booking.event}</Text>
                    <Text className="text-gray-500 text-sm">{booking.time}</Text>
                  </View>
