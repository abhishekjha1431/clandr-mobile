...
export default function EditEventModal({ visible, event, onClose, onSave, onDelete }: EditEventModalProps) {
      ...
      <View className="flex-1 bg-gray-50">
        ...
        <ScrollView className="flex-1 p-4">
          ...
          <View className="bg-white rounded-3xl shadow-lg p-6 mb-4">
            <Text className="text-lg font-bold text-gray-800 mb-3">Event Title</Text>
            <TextInput
              value={title}
              ...
              multiline={false}
            />
            <Text className="text-gray-500 text-sm mt-2">The name users will see when booking</Text>
          </View>

          {/* Duration */}
          <View className="bg-white rounded-3xl shadow-lg p-6 mb-4">
            <Text className="text-lg font-bold text-gray-800 mb-3">Duration</Text>
          ...
          <View className="bg-white rounded-3xl shadow-lg p-6 mb-4">
            ...
            />
            <Text className="text-gray-500 text-sm mt-2">Optional description of the event</Text>
          </View>
          ...
          <View className="bg-white rounded-3xl shadow-lg p-6 mb-6">
            <View className="flex-row items-center justify-between">
              <View className="flex-1">
                <Text className="text-lg font-bold text-gray-800 mb-1">Active</Text>
                <Text className="text-gray-500 text-sm">Inactive events will not be visible for users to book</Text>
              </View>
              <TouchableOpacity
                onPress={() => setActive(!active)}
                className={`w-14 h-8 rounded-full p-1 ${active ? 'bg-blue-500' : 'bg-gray-300'}`}
              >
                <View className={`w-6 h-6 bg-white rounded-full shadow-sm transition-all ${active ? 'ml-auto' : ''}`} />
              </TouchableOpacity>
            </View>
          </View>
        </ScrollView>
